function figures = save_viewclusters_figure(grouping, varargin)
%VIEWCLUSTERS
 
  PoincareHypDisks                  = true;         % default: true
  minEdgeCrossings                  = true;         % default: true
  graphOpts.undirected.visibility   = true;         % default: true
  graphOpts.undirected.handle       = true;         % default: true
  graphOpts.directed.visibility     = true;         % default: true
  graphOpts.directed.handle         = true;         % default: true
  txt                               = [];
  assignopts(who, varargin);
 
  if ~(graphOpts.undirected.visibility || graphOpts.undirected.handle ||...
       graphOpts.directed.visibility || graphOpts.directed.handle)
    error('Please specify a graph to be generated');
  end
  ...
  if (graphOpts.undirected.visibility || graphOpts.undirected.handle)
    ...
    if ~(graphOpts.undirected.visibility)
      set(figures(end),'Visible','off')
    end % if ~(graphOpts.undirected.visibility)
    if ~(graphOpts.undirected.handle)
      figures(end)              = [];
    end % if ~(graphOpts.undirected.handle)
      ...
      for j=1:nCliques(i)
        ...
        if (minEdgeCrossings)
          for k=1:size(edges,1)
            adjacencyMatrix(edges(k,1),edges(k,2))...
                                = 1;
            adjacencyMatrix(edges(k,2),edges(k,1))...
                                = 1;
    ...
    set(gcf,'Visible','on')
    uisave(gcf,'Figure_Undirected_Identifier.fig');
    set(gcf,'Visible','off')
    
    
  end % if (graphOpts.undirected.visibility || graphOpts.undirected.handle)
  
  if (graphOpts.directed.visibility || graphOpts.directed.handle)
    % number of directed subgraphs
    ...
    % plot nodes
    figures                     = [figures, show];
    if ~(graphOpts.directed.visibility)
      set(figures(end),'Visible','off')
    end % if ~(graphOpts.directed.visibility)
    if ~(graphOpts.directed.handle)
      figures(end)              = [];
    end % if ~(graphOpts.directed.handle)
    ...
    % save current figure
    set(gcf,'Visible','on')
    uisave(gcf,'Figure_Directed_Identifier.fig');
    set(gcf,'Visible','off')
